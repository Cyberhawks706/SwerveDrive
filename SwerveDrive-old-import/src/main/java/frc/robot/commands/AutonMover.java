...
public class AutonMover extends CommandBase {
	...
	private static int initialPlaced = 0;
	...
	public void execute() {
		...
		// x is fwd/back
		if(initialPlaced < 50) {
			Components.sparkClawTilt.setPos(0.9);
			initialPlaced++;
			return;
		}
		else if (initialPlaced < 350) {
			scoreTopCone();
			initialPlaced++;
			return;
		}
	...
	public void pickupCone() {
		...
		if (Math.abs(swerveSubsystem.getHeading() - pickupHeading) > 5 && !turned) { // we need to turn it turn it
			// if(timer < 800){
			// target = new Transform3d(new Translation3d(0,-0.65,0), new
			// Rotation3d(0,0,0));
			// Components.sparkIntake.setPower(0);
			...
			ChassisSpeeds chassisSpeeds = ChassisSpeeds.fromFieldRelativeSpeeds(0, -0.3,
					(swerveSubsystem.getHeading() - pickupHeading) / 350, swerveSubsystem.getRotation2d());
			...
			if (targetDistance > 50) {
				...
				target = new Transform3d(new Translation3d(0, targetDistance / 100, 0),
						new Rotation3d(0, 0, targetAngle));
				Components.sparkIntake.setPower(0);
				// scoreTopCone();
			} else {
			...
			SwerveModuleState[] swerveModuleStates = SwerveConstants.kDriveKinematics
					.toSwerveModuleStates(chassisSpeeds);
			swerveSubsystem.setModuleStates(swerveModuleStates);
		} else if (turned && pickedUp && !arrived) {
			Components.sparkIntake.setPower(0);
			if (Math.abs(swerveSubsystem.getHeading() - startingAngle) > 5) { // need to turn back
				ChassisSpeeds chassisSpeeds = ChassisSpeeds.fromFieldRelativeSpeeds(0, -0.3,
						(swerveSubsystem.getHeading() - startingAngle) / 200, swerveSubsystem.getRotation2d());
				SwerveModuleState[] swerveModuleStates = SwerveConstants.kDriveKinematics
						.toSwerveModuleStates(chassisSpeeds);
				swerveSubsystem.setModuleStates(swerveModuleStates);
			} else {
	...
	public void pickup() {
		...
		if (((clawTiltMotorPos + clawTiltSpeed * 0.5) > 0.43
				...
				|| ((clawTiltMotorPos + clawTiltSpeed * 0.5) < 20.75
						&& (clawTiltMotorPos + clawTiltSpeed * 0.5) > clawTiltMotorPos))) {
	...
	public void scoreTopCone() {
		...
		double desiredClawPos = 17.66; //2
		double desiredRLift = 2.82;//2.99
		double desiredFLift = 2.906;//2.955
		 ...
		 *
		 *
		 * if(Math.abs(clawTiltSpeed) > 1) clawTiltSpeed = clawTiltSpeed /
		 * Math.abs(clawTiltSpeed);
		 *
		 * if( ((clawTiltMotorPos + clawTiltSpeed * 0.5) > 0.43 && (clawTiltMotorPos +
		 * clawTiltSpeed * 0.5 ) < clawTiltMotorPos ||((clawTiltMotorPos + clawTiltSpeed
		 * * 0.5) < 20.75 && (clawTiltMotorPos + clawTiltSpeed * 0.5 ) >
		 * clawTiltMotorPos ))) {
		 * clawTiltMotorPos += clawTiltSpeed * 2;
		 * }
		 *
	...
	private Transform3d calculateTagPos(int camId, int tagId) {
		...
		double x = Double.valueOf(
				table.getSubTable("processed" + camNum).getSubTable("tag" + tagNum).getEntry("tx").getString("0"));
		double y = Double.valueOf(
				table.getSubTable("processed" + camNum).getSubTable("tag" + tagNum).getEntry("ty").getString("0"));
		double z = Double.valueOf(
				table.getSubTable("processed" + camNum).getSubTable("tag" + tagNum).getEntry("tz").getString("0"));
		// double yaw = table.getSubTable("processed" + camNum).getSubTable("tag" +
		// tagNum).getEntry("yaw").getDouble(0.0);
		double yaw = 0;
		z--;
	...
	private ChassisSpeeds calculateSpeedsToTarget(Transform3d target) {
		...
		// double rotMultiplier = Math.abs(angularDistance /
